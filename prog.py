...
params = [100, 10, 0, 30, 300, 100] # параметры для теста программы (после теста будут убраны)
params = list(map(float, params)) # от пользователя параметры пришли строкой нужно перевести в числа
dX, f_cr, k, theta_degree, Zm, Wm = params # создание переменных из массива параметров


f_cr *= 1000000 # МГц в Гц
theta = radians(theta_degree) # градусы в радианы
Zm *= 1000 # метры в километры
Wm *= 1000 # метры в километры
